...
class Torrents(DatabaseModel):
    ...
    added_time = peewee.TimestampField(resolution=1, utc=True, default=lambda: datetime.now(timezone.utc))
    delete_time = peewee.TimestampField(resolution=1, null=True, utc=True, default=None) # resolution=1 means seconds
    ...
    class Meta:
        indexes = (
            ...
            (('delete_time',), False),  # (delete_time,), not unique, make queries for non-deleted torrents faster
        )
...
class TorrentsComputed(DatabaseModel):
    ...
    torrent = peewee.ForeignKeyField(  # column_name = 'torrent_id'
        ...
        constraints=[peewee.SQL("PRIMARY KEY")] # Virtual PK for Peewee's internal logic
    )
    torrent_id = peewee.IntegerField()
